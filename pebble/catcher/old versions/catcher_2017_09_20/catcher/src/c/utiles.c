...
void pon_tiempo_en_layer(TextLayer *playertiempo,uint32_t ptiempo) {
    ...
    snprintf(buffer_tiempo_texto,80,"%lu",ptiempo);
...
//cadena tiene que tener suficiente espacio para que una vez reemplazadas
//las cadena de pcadenaareemplazar por pcadenadestino quepa la cadena.
void reemplazar(char *pcadena,char *pcadenaareemplazar, char *pcadenadestino) {
    int llencadenareemplazar;
    int i;
    int llenpcadena;
    int llenpcadenadestino;
    int loffsetlong;
    char *p,*pdest;
    char *lcadenabuffer;
    bool lreemplazado;
    
    llencadenareemplazar=strlen(pcadenaareemplazar);
    llenpcadenadestino=strlen(pcadenadestino);
    loffsetlong=llenpcadenadestino-llencadenareemplazar;
    llenpcadena=strlen(pcadena);    
    i=0;    
    lcadenabuffer=malloc(llenpcadena+1);
    strcpy(pcadena,lcadenabuffer);
    p=lcadenabuffer;
    pdest=pcadena;
    do {
        lreemplazado=false;
        if ((i+llencadenareemplazar)<llenpcadena) {
            if (strncmp(p,pcadenaareemplazar,llencadenareemplazar)==0) {
                strcat(pcadena,pcadenadestino);
                lreemplazado=true;
                p+=llencadenareemplazar;
                pdest+=llencadenadestino;
                llenpcadena+=loffsetpcadena;
                i+=llencadenareemplazar;
            }
        }
        if (!lreemplazado) {
            *pdest=*p;
