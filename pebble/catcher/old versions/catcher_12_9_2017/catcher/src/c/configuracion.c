...
APP_LOG(APP_LOG_LEVEL_DEBUG, "función lee_config");
lVers=NUM_VERSION_CONFIG; //por defecto, ultima versión de config
...
if (lVers==1)    {
    ...
    if (persist_exists(KEY_CONFIG))  {
        ...
        APP_LOG(APP_LOG_LEVEL_DEBUG, "lee_config guardada");
    }
    else {
        //si no tiene configuración, crear una en blanco
