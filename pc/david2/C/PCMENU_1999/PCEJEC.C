...
int sa;
char temp[MAXPATH+1],*p;


borratodapant();
veaxy(0,0);
copiastr(temp,argv[0]);
p=busca(temp,0);
while (*p!='\\') --p;
++p;
copiastr(p,"PCMENU.EXE");
sa=ejecuta(argv); //argv es para saber donde está PCMENU.TEM
...
if (sa!=0) {
  escribe (mensajes[4]);
  esperatecla();  }
//a spawnl se le debe pasar al menos un parámetro para que funcione
if (spawnl(P_OVERLAY,temp,argv[0],NULL)==-1) {
	escribe(mensajes[5]);
	return 1;
  }
return 0;
...
//**argv es para saber donde está PCMENU.TEM
int res,leer,unid;
