...
_x=xsal+(unsigned char)(strchr(unidlogicas,cogedisco()+'A')-
...
if (teclapul!=ESC) {
		  ...
		  cambiadir("\\"); //por si hay problemas en el
		  //cambio de discos
...
bandera=0; //si =0 utiliza las funciones del DOS, si es 1 utiliza cogerinfodisco
if (unidad[uni].atribs.noaccesodirecto==0) bandera=1;
if (bandera==1) {
...
sprintf(info,"UNIDAD %c",uni+'A');
ventanas[13].titulo=info;
...
if (bandera) {
  ...
  capacidad-=(((unsigned long)capacid.sectorfat*
  capacid.copiasfat)+(unsigned long)capacid.sectoresespreserv+
  (unsigned long)capacid.secreservcomie);
  capacidad*=capacid.bytsector;
  capacidad-=(unsigned long)capacid.entradasdirprin*32;
  }
else {
...
if (bandera)
  ...
  sprintf(info,"%u",libred.df_sclus*libred.df_bsec);
centrar(7,0,info);
if (bandera) {
