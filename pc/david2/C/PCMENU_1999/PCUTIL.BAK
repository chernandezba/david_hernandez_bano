...
{SISALIR,"PCDISCO NO PUEDE SOPORTAR NINGUNA UNIDAD DE LAS DISPONIBLES"},
};
...
{1,3,24,21,siguarda,BORDER,"",NULL},/*PANTALLA DE INFORMACION DEL DISCO*/
...
static struct estructarb *actualiza_ptr_con_dir_act(char *diract);
/*-------------------------------------------------------------------------*/
...
sizeof(ESTRUCTARB));
npuntarb++;
sprintf(_____temp,"%u",npuntarb-1);
centrar(3,0,_____temp);
...
if (utilizachdir==TRUE) {
  //cambiadir en estos casos llama a chdir
  if ((__er=cambiadir(arbsub.ff_name))!=0) {
	 //si hay error al cambiar de directorio
	 //como al cambiar de directorio en un CD-ROM
	 //con un nombre de directorio largo usando
	 //su nombre corto bajo windows 95
	 //va saliendo del arbol
	 return ;
	 }
  }
...
static struct estructarb *actualiza_ptr_con_dir_act(char *diract) {
char huge *puntact,*p;
char dir2[14]; //directorio sin \
unsigned char l;
struct estructarb *arbol2=arbol;
...
while (*p!=0) {
  l=0;
  p=(char *)puntact;
  arbol2=arbol2->subdir;
  if (arbol2==NULL) break;
  while (*p!='\\' && *p!=0) {
	 ++l;
	 ++p;
	 }
  strncpy(dir2,(char *)puntact,l);
  dir2[l]=0;
  while (strcmp(dir2,arbol2->nombredir) && dir2!=NULL)
	 arbol2=arbol2->proxdir;
  if (arbol2==NULL) {
	 encontrado=FALSE;
	 break;
	 }
...
if (encontrado==FALSE) 
  //si no lo encuentra devuelve el directorio raiz
  return arbol;
return arbol2;
}
/*----------------------------------------------------------------------*/
void buscararbol(int _unidad)
  {
...
arbol->x=x0;
arbol->y=y0;
sprintf(arbol->nombredir,"%c:\\",(unsigned char)_unidad+'A');
arbol->atributo=fich.ff_attrib;
arbol->clusterinicial=0;
if (pru2==0) {
  if ((er=ponesubdirs(arbol))!=0 && !unidad[_unidad].
  atribs.noaccesodirecto) errordisco(er);
  }
else if (pru2!=-1 && !unidad[_unidad].atribs.
  noaccesodirecto) errordisco(er);
arbol=farrealloc(arbol,gastado+sizeof(struct estructarb));
cerrarven(&ventanas[7]);
cambiadisco(diski);
if (cambiadir(bufint)!=0)
  //si ha habido error al cambiar al directorio anterior
  cambiadir("\\");
pon_atributo(at);
cambiarven(ventsa);
arbolptr=actualiza_ptr_con_dir_act(unidad[_unidad].diractual_disco);
//ajusta coordenadas de los subdirectorios que aparecen en la
//ventana de subdirectorios
if ((arbolptr->y-arbol->y)>altoarbol) {
  arboly=arbolptr->y;
  yrel=2;
  }
else {
  arboly=arbol->y;
  yrel=arbolptr->y-arbol->y+2;
  }
}
/*------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------*/
void mostrardirectorio(int _unidad) {
...
if ((ficherosdir=(struct filee huge *)farmalloc(libre2))==NULL)
  ponerror(1);
if ((er=meteficheros(ficherosdir))!=0 &&
!unidad[_unidad].atribs.noaccesodirecto) errordisco(er);
if (!totalfich)
  liberamemfich();
else
...
pon_atributo(colorf2(ARBOLV));
if (etiqueta_disco[0]!=0)
  centrar(ventana_alto()+1,0,etiqueta_disco);
else
  centrar(ventana_alto()+1,0,"Sin etiqueta");
...
void sel_fich(void);
char pathtemp[MAXPATH+1],pathtemp2[MAXPATH+1];
...
cogerdiractual(pathtemp,MAXPATH);
getcwd(pathtemp2,MAXPATH);
chdir(pathtemp); //para que cambie realmente de directorio
olddisk=cogedisco();
__setcursortype(_NORMALCURSOR);
res=system(comando);
if (condicion==1 && res!=-1) {
  writestr(1,25,0,"PULSE TECLA PARA VOLVER A PCMENU");
  tecla();
  };
regs.h.ah=0x00;
...
chdir(pathtemp2);
cambiadisco(olddisk);
if (cambiadir(pathtemp)!=0)
  //si ha habido error al cambiar al directorio anterior
  //puede ser debido a error del disco o a que el directorio
  //ha sido borrado
  cambiadir("\\");
